...
def portfolio_Attribution():
    ...
    if okbutton:
        ...
        daily_shares_changed.to_excel(excel_writer = 'excel_Data.xlsx', sheet_name = 'Daily Transactions')
        ...
        positions_filled_forward.to_excel(excel_writer = 'excel_Data.xlsx', sheet_name = 'Postions')
        ...
        prices.to_excel(excel_writer = 'excel_Data.xlsx', sheet_name = 'prices')
        #st.dataframe(prices)
        ...
        weights = position_value.iloc[:,0:-1].divide(position_value.iloc[:,-1], axis=0)
        ...
        with metricscol5:
            ...
            beta = (correlation_with_benchmark.corr().iloc[0,1])
            st.metric(label="Beta", value='', delta=f"{round(beta,2)}", width='content', delta_color='inverse')        
